...
export class ServerInfoCommand extends SwanCommand {
  ...
  private async _exec(
    ...
    if (server.ip)
      embed.addFields({
        name: embedMessages.ip,
        value: `\`${server.ip}${server.port ? `:${server.port}` : ''}\``,
        inline: true,
      });
    if (server.players?.online && server.players?.max)
      embed.addFields({
        name: embedMessages.players,
    ...
    if (server.hostname)
      embed.addFields({
        name: embedMessages.hostname,
        value: server.hostname,
        inline: true,
      });
    if (server.software)
      embed.addFields({
        name: embedMessages.software,
        value: server.software,
        inline: true,
      });
    if (server.plugins?.names)
      embed.addFields({
        name: embedMessages.plugins,
        value: server.plugins.names.length.toString(),
        inline: true,
      });
    if (server.mods?.names)
      embed.addFields({
        name: embedMessages.mods,
        value: server.mods.names.length.toString(),
        inline: true,
      });

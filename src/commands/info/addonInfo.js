...
import { SkripttoolsAPI, config } from '../../main';
...
class AddonInfo extends Command {
	constructor() {
		super('addonInfo')
		this.usage = `${config.bot.prefix}addon-info `;
		this.exemple = ['addon-info skquery-lime', 'addonsinfos -list'];
	...
	async execute(message, args) {
		if (args.length < 1) {
			...
			const addons = await SkripttoolsAPI;

...
class AutomaticMessages extends Command {
  ...
  async execute(message, args) {
    ...
    if (args.length === 0) message.channel.send(discordError(this.config.noArg.replace('%s', `\n - ${Object.keys(this.config.messages).join('\n - ')}\n`), message));
    ...
    } else message.channel.send(discordError(this.config.invalidMessage, message));
  }
}

...
Les durées des sanctions peuvent être écrites en français ou en anglais. Elle doit être construite de la manière suivante : `<nombre><indicatif>`.
...
S'il n'est pas bannit, alors ça va créer le bannissement, sinon, s'il est déjà bannit, ça va mettre à jour la durée en la **remplaçant**
par la durée spécifiée. La raison entrée sera alors le motif pour avoir mis à jour la durée.

##### Annulation

Pour annuler le bannissement d'un membre, il faut utiliser la commande `.unban` (ou `.deban`), suivit de l'utilisateur. Vous pouvez aussi, optionnellement, ajouter une raison pour ce dé-bannissement.
Cette commande fonctionne aussi bien pour les bannissements permanants et temporaires, seulement vous ne pourrez pas mentionner l'utilisateur pour les bannissements permanants car il n'est pas présent dans la guilde.

Modèle : `.unban <@mention | pseudo | ID> [raison]`

#### Précisions

- Si, pendant un bannissement temporaire, l'utilisateur quitte le discord, il sera automatiquement bannit de manière permanante.
...
aucun message dans son salon privé à la fin de la sanction, il sera bannit définitivement, sinon il sera simplement débannit.

Voici donc le modèle d'une commande :\
`.ban <@mention | pseudo | ID> <durée> <raison> [--autoban]`
...
Un mute empêche un membre de pouvoir écrire des messages dans un salon spécifique.
...
alors ça va créer le mute, sinon, s'il est déjà mute, ça va mettre à jour la durée en la **remplaçant** par la durée spécifiée. La raison entrée
sera alors le motif pour avoir mis à jour la durée.

##### Annulation

Pour annuler le mute d'un membre, il faut utiliser la commande `.unmute` (ou `.demute`), suivit de l'utilisateur. Vous pouvez aussi, optionnellement, ajouter une raison pour ce dé-mute.
...
- Si un membre quitte le discord en étant mute, puis qu'il rejoint la guilde est que le bot est hors-ligne, alors il n'aura plus son rôle "Mute" et pourra continuer à envoyer des messages, même si le bot revient en ligne.
...
Un kick permet d'expulser un membre de la guilde. Il peut ensuite rejoindre la guilde via une invitation.
...
Un warn permet de donner un avertissement à un membre.
...
- Après 2 avertissements, le membre sera automatiquement bannit pendant 4 jours.
...
Chaque sanction est une action, qui hérite de la classe ModerationAction. Il faut donner un paramètre un object ModerationData, qui contient les informations relatifs à la sanction.
...
Diverses fonctions utilitaires relatifs à la modération se trouvent dans la classe ModerationHelper.

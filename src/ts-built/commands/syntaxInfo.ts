...
async function sendEmbed(message: Message, data: any) {
	...
	if (data.example) {
		let ex: string = `${conf.embed.exampleDesc.replace('%s', data.example.example_code).replace('%l', '	‌')}\n`;
...
class SyntaxInfo extends Command {
	...
	usage: string = `syntax-info <syntaxe> [-a:<addon>] [-t:<type>] [-id:<ID skripthub>]`;
	examples: string[] = ['syntax-info join', 'doc tablist', 'sinfo tablist -addon:skrayfall', 'sinfo -id:2000', 'doc join -type:event'];
	channels: string[] = ['*'];
	regex: RegExp = /(?:s(?:yntax)?-?infos?|doc(?:umentation)?s?)/gimu;
	...
	execute = async (message: Message, args: string[]): Promise<void> => {
		if (args.length < 1) {
			...
			if (matchingSyntaxes.length === 0) {
				...
				if (results - 10 > 0) msg = <Message> await msg.edit(`${msg.content}\n...et ${results - 10} de plus...`);
				await msg.react('❌');

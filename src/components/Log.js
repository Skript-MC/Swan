...
export async function modLog(info, guild) {
    ...
    if (info.finish && info.duration && info.finish !== "") embed.addField(":stopwatch: Durée", `${info.duration}\nExpire ${info.finish}`, true);
    else if (info.duration) embed.addField(":stopwatch: Durée", `${info.duration}`, true);

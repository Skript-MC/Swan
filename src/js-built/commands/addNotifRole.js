...
class AddNotifRole extends Command {
	...
	name = 'Ajout role notification';
	description = config.messages.commands.addNotifRole.description;
	examples = ['add-notif-role', 'toggle-notif-role'];
	regex = /(|give|ask|toggle)-?notif()?-?role/gmui;

	execute = async (message, args) => {
		message.delete();
		let role = message.guild.roles.find(r => r.name === roleName),
			targetUser = await message.guild.fetchMember(message.author);

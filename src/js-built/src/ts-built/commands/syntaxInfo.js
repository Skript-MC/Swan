...
class SyntaxInfo extends Command_1.default {
    constructor() {
        ...
        this.usage = `syntax-info <syntaxe> [-a:<addon>] [-t:<type>] [-id:<ID skripthub>]`;
        this.examples = ['syntax-info join', 'doc tablist', 'sinfo tablist -addon:skrayfall', 'sinfo -id:2000', 'doc join -type:event'];
        this.channels = ['*'];
        ...
        this.execute = (message, args) => __awaiter(this, void 0, void 0, function* () {
            if (args.length < 1) {
                ...
                if (arg === '')
                    return Messages_1.discordError(conf.invalidCmd, message);
                ...
                if (matchingSyntaxes.length === 0) {
                    ...
                    yield msg.edit(`${results} élements trouvés pour la recherche \`${args.join(' ')}\`${search.length > 0 ? ` avec comme paramètres \`${search.join(', ')}\`` : ''}. Quelle syntaxe vous interesse ?\n:warning: **Attendez que la réaction :x: soit posée avant de commencer.**`);

...
export class LoadSkriptMcSyntaxesTask extends Task {
  public override async run(): Promise<void> {
    ...
    try {
      ...
      const allSyntaxes: SkriptMcDocumentationSyntaxResponse[] = await axios(
        `${apis.skriptmc}/documentation/syntaxes?${token}`,
      ).then((res) => res?.data);
      const allAddons: SkriptMcDocumentationFullAddonResponse[] = await axios(
        `${apis.skriptmc}/documentation/addons?${token}`,
      ).then((res) => res?.data);

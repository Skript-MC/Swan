...
export async function randomCommand(commands, withoutPerms = true) {
  for (;;) {
...
export async function randomActivity(client, commands, prefix) {
  if (!client.config.activated) return { activity: { name: 'Désactivé.', type: 'WATCHING' }, status: 'dnd' };
  ...
  if (random === 1) status = { activity: { name: 'développer en Skript', type: 'PLAYING' }, status: 'idle' };
  if (random === 2) status = { activity: { name: `${prefix}aide | Skript-MC`, type: 'WATCHING' }, status: 'online' };
  if (random === 3) status = { activity: { name: `${prefix}help ${await randomCommand(commands)}`, type: 'WATCHING' }, status: 'online' };
  return status;
}

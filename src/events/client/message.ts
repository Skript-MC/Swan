...
export default class MessageEvent extends Event {
  ...
  private async _addReactionsInIdeaChannel(message: GuildMessage): Promise<boolean> {
    ...
    if (message.channel.id === settings.channels.idea) {
      try {
        ...
        this.context.logger.error('Unable to add emojis to the idea channel.');
        this.context.logger.info(`Has "ADD_REACTION" permission: ${message.guild.me?.permissionsIn(message.channel).has(Permissions.FLAGS.ADD_REACTIONS)}`);
        this.context.logger.info(`Emojis added: "${settings.emojis.yes}" + "${settings.emojis.no}"`);
        this.context.logger.info(`Idea channel ID/Current channel ID: ${settings.channels.idea}/${message.channel.id} (same=${settings.channels.idea === message.channel.id})`);
  ...
  private async _quoteLinkedMessage(message: GuildMessage): Promise<boolean> {
    ...
    if (message.content.startsWith(settings.bot.prefix))
      // || this.context.client.commandHandler.hasPrompt(message.channel, message.author))
      return false;

    // Quote a linked message.
    if (!MessageLinkRegex.test(message.content))
      return false;

    const quotes: Array<{ channelId: string; messageId: string }> = [];
    let text = message.content;
    while (MessageLinkRegex.test(text)) {
      const [full, channelId, messageId] = MessageLinkRegex.exec(text)!;
      quotes.push({ channelId, messageId });
      text = text.replace(full, '');
    }

    for (const quote of quotes) {
      const channel = await this.context.client.channels.fetch(quote.channelId).catch(nullop);

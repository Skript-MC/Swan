...
export async function end(pollId: string, stopped = false): Promise<void> {
  ...
  if (poll.questionType === QuestionType.Yesno) {
    const yes = poll.votes[miscellaneous.pollReactions.yesno[0]].length;
    const no = poll.votes[miscellaneous.pollReactions.yesno[1]].length;
    ...
    for (let i = 0; i < Object.keys(poll.votes).length; i++) {
      const reac = miscellaneous.pollReactions.multiple[i];
